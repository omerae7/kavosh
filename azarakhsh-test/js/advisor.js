/* =========================================================================
   آذرخش · میزِ مشاورِ فنی
   موتور گفتگوی محلی: تشخیص قصد، پیشنهاد رنگ، محاسبهٔ متراژ و ثبت در پرونده.
   کاملاً آفلاین روی همین صفحه اجرا می‌شود؛ هیچ داده‌ای جایی فرستاده نمی‌شود.
   در این نسخه، پاپ‌آپ به میزِ کارِ آزمایشگاه تبدیل شده: کشوی کنسول باز
   می‌شود، خطِ اسکنر یک بار روی سربرگ می‌گذرد و گفتگو شروع می‌شود.
   ========================================================================= */
(function () {
  'use strict';
  ...
  const KEY = 'azarakhsh-test-chat-v1';
  ...
  function respond(raw) {
    ...
    if (hits(t, ['ممنون', 'مرسی', 'سپاس', 'لطف کرد', 'دمت گرم'])) {
      return {
        ...
        chips: ['نمونهٔ رایگان می‌خواهم', 'پرونده‌ام را ببین', 'شمارهٔ تماس کارخانه']
      };
    }
    ...
    if (wantsCalc || looksLikeDims) {
      if (looksLikeDims) {
        ...
        return {
          ...
          cards: [target.id],
          add: { id: target.id, qty: Math.round(est.billed * 2) / 2, label: `ثبت ${faFloat(est.billed)} ${target.unit} در پرونده` },
    ...
    if (direct && hits(t, ['قیمت', 'چند', 'مشخصات', 'درباره', 'توضیح', 'چیه', 'چطور']) > 0) {
      ...
      return {
        ...
        cards: [direct.id],
        add: { id: direct.id, qty: 1, label: 'ثبت در پرونده' },
    ...
    if (hits(t, ['سبد', 'سفارش', 'خرید', 'ثبت کن', 'فاکتور بده'])) {
      ...
      if (!tot.rows) {
        return {
          text: 'پروندهٔ سفارش شما هنوز خالی است. اگر بگویید چه رنگی می‌خواهید یا متراژ نما چقدر است، همین‌جا برایتان اضافه می‌کنم.',
          chips: ['برای نمای مدرن چه رنگی؟', '۱۲ در ۳', 'ارزان‌ترین گزینه']
        };
      }
      return {
        text: `در پروندهٔ شما **${fa(tot.rows)} قلم** به ارزش **${toman(tot.sum)}** ثبت شده است.\nبرای دریافت پیش‌فاکتور رسمی، پرونده را باز کنید و اطلاعات تماس را وارد کنید.`,
        action: 'cart',
  ...
  async function addBot(reply) {
    ...
    if (reply.cards && reply.cards.length) {
      ...
      $$('[data-open]', box).forEach(b =>
        b.addEventListener('click', () => { location.href = 'product-' + b.dataset.open + '.html'; }));
    }

    if (reply.add) {
      const btn = document.createElement('button');
      btn.className = 'chat__add';
      btn.innerHTML = '<svg><use href="#i-plus"/></svg>' + esc(reply.add.label);
      btn.addEventListener('click', () => {
        Cart.add(reply.add.id, null, reply.add.qty);
        btn.disabled = true;
        btn.classList.add('is-done');
        btn.innerHTML = '<svg><use href="#i-check"/></svg> در پرونده ثبت شد';
      });
      body.appendChild(btn);
    }

    if (reply.action === 'cart') setTimeout(() => { const b = $('#fileBtn') || $('#dockFile'); if (b) b.click(); }, 700);
    if (reply.action === 'calc') setTimeout(() => { close(); go('#issue'); }, 900);
    if (reply.action === 'contact') setTimeout(() => { close(); go('.foot'); }, 900);
  ...
  function init() {
    ...
    btn.addEventListener('click', e => { e.preventDefault(); ask(box.value); });
    const composer = $('#composer');
    if (composer) composer.addEventListener('submit', e => { e.preventDefault(); ask(box.value); });
    const rst = $('#chatReset');
  ...
  function open(trigger, question) {
    const box = POP();
    if (!box) return;
  ...
  /* اسکرول نرم اگر لایهٔ حرکت حاضر بود */
  function go(sel) {
    if (window.AZMotion) AZMotion.goTo(sel);
    else { const el = $(sel); if (el) el.scrollIntoView({ behavior: 'smooth' }); }
  }

  window.AZAdvisor = { init, open, close, ask, reset, respond };
